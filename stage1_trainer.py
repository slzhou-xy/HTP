...
class Trainer:
    def __init__(self,
        ...
        self.model, self.optimizer, self.scheduler, self.train_loader, self.eval_loader = self.accelerator.prepare(
            self.model, self.optimizer, self.scheduler, self.train_loader, self.eval_loader
        )
    ...
    def eval(self, epoch):
        ...
        pbar = tqdm(self.eval_loader, ncols=110, disable=not self.accelerator.is_main_process)
        ...
        for batch_idx, batch_data in enumerate(pbar):
            ...
            percent_loss = rtn['percent_loss'].item()
            dxy_loss = rtn['dxy_loss'].item()
            ...
            if self.writer is not None:
                write_scalar(self.accelerator, self.writer, 'eval/loss', loss, self.eval_global_step)
                write_scalar(self.accelerator, self.writer, 'eval/recon_loss', recon_loss, self.eval_global_step)
                write_scalar(self.accelerator, self.writer, 'eval/quant_loss', quant_loss, self.eval_global_step)
                write_scalar(self.accelerator, self.writer, 'eval/percent_loss', percent_loss, self.eval_global_step)
                write_scalar(self.accelerator, self.writer, 'eval/dxy_loss', dxy_loss, self.eval_global_step)

            pbar.set_description(f"[Eval  {epoch+1}/{self.n_epochs}]")
        ...
        avg_info = merge_info(info_list)
        avg_info_str_list = [', '.join(f"{k}: {v:.8f}" for k, v in info.items()) for info in avg_info]

        log_str(self.accelerator,
                f"Eval  Ep {epoch+1}/{self.n_epochs} | loss: {avg_loss:.8f}, recon: {avg_recon_loss:.8f}, quant: {avg_quant_loss:.8f}, percent: {avg_percent_loss:.8f}, dxy: {avg_dxy_loss:.8f}")

        for i, avg_info_str in enumerate(avg_info_str_list):
            log_str(self.accelerator, f"Eval  Ep {epoch+1}/{self.n_epochs}|RQ_Info_{i + 1}: {avg_info_str}")
